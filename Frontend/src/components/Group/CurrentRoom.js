...
export default function CurrentRoom({ room }) {
  const [showModal, setShowModal] = useState(false);

  return (
    <Fragment>
      <header className="container d-flex justify-content-between align-items-end mt-3 pr-5">
        <div className="d-flex justify-content-start align-items-end py-2 h-75">
          <P className="p-1 m-0 font-weight-bolder text-capitalize">{room}</P>
        </div>
        <div className="d-flex justify-content-end py-2 h-75">
          <ButtonEnd
            type="button"
            className="btn mx-1"
